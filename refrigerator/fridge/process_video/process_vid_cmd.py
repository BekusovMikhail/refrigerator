...
def get_detection_res(client, img_preprocessed):
    ...
    detection_response = client.infer(
        model_name="hand_detection", inputs=[detection_input]
    )
...
def main(args):
    ...
    while cap.isOpened():
        ...
        if ret == True:
            imgs.append(frame)
            img_preprocessed, shapes_info = img_detector_preprocess(
                cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
            )
            boxes, scores, labels = get_detection_res(client, img_preprocessed)
            filtered_boxes, filtered_scores, filtered_labels = filter_boxes(
                boxes, scores, labels, shapes_info
            )
            objs = find_objects(
                filtered_boxes, filtered_scores, filtered_labels, shapes_info
            )
            ...
            if result is not None:
                ...
                for i in range(result[0], result[1]):
                    ...
                    if (
                        obj["object"] is not None
                        and obj["object"]["score"] > max_obj_conf
                    ):
                ...
                img_cropped = imgs[max_obj_id][
                    int(obj["object"]["bbox"][1]) : int(obj["object"]["bbox"][3]),
                    int(obj["object"]["bbox"][0]) : int(obj["object"]["bbox"][2]),
                    :,
                ]
                img_preprocessed = img_classificator_preprocess(
                ...
                final_tracking_results.append({"action": action, "name": name})
    ...
    for res in final_tracking_results:
        if res["action"] == "add":
            os.system(
                f"python add_rm_product_from_DB.py --product_name {res['name']} --fridge_id {args.fridge_id}"
            )
        else:
            os.system(
                f"python add_rm_product_from_DB.py --product_name {res['name']} --rm --fridge_id {args.fridge_id}"
            )
        # print(res)
